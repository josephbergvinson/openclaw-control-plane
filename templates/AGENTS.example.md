...
- Before asking the operator for factual input, use established facts, retrieve the relevant available records and perform reasonable derivations. An unsearched field or an inconclusive memory snippet is not a missing user input. A request for exact questions means questions about the remaining gaps, not a fresh questionnaire about already established information. Follow the retrieval-before-clarification mechanics in `TOOLS.md`; ask only for unresolved choices, material conflicts, inaccessible evidence or genuinely current personal confirmation.
...
- Send a new checkpoint at meaningful milestones and at least once every three minutes while substantial work remains active. Editing an earlier checkpoint does not replace the next visible update. Use the existing originating-thread message route for a new nonfinal checkpoint (TOOLS §Work checkpoints); keep the admitted final-delivery path available. If progress has stopped for input, approval, tool failure, or an external dependency, say so within two minutes and name the real wait.
